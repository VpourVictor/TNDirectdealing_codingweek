            ...
            for (int i = 1; i <= Post.getNbPosts(); i++){
                ...
                if (json.get("post " + i) != null) {
                    ...
                    if (type.equals("service")) {
                        ...
                        posts.add(new Service(json.getString("description"), json.getString("title"), json.getString("authorEmail"), startDate, endDate, jsonToAdress(json.getJSONObject("address")), image, state, json.getString("descriptionService")));
                    }
                    else if (type.equals("tool")) {
                        System.out.println("add tool");
                        posts.add(new Tool(json.getString("description"), json.getString("title"), json.getString("authorEmail"), startDate, endDate, jsonToAdress(json.getJSONObject("address")), image, state, json.getString("stateTool")));
                    }
                }
            }
